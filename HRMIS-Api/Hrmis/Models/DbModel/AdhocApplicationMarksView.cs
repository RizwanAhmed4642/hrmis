
//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------


namespace Hrmis.Models.DbModel
{

using System;
    using System.Collections.Generic;
    
public partial class AdhocApplicationMarksView
{

    public int Id { get; set; }

    public Nullable<int> Application_Id { get; set; }

    public Nullable<int> ApplicantQualification_Id { get; set; }

    public Nullable<int> Marks_Id { get; set; }

    public Nullable<double> Marks { get; set; }

    public Nullable<double> Percentage { get; set; }

    public string Remarks { get; set; }

    public Nullable<bool> Error { get; set; }

    public Nullable<bool> IsActive { get; set; }

    public Nullable<System.DateTime> CreatedDate { get; set; }

    public string UserId { get; set; }

    public string CreatedBy { get; set; }

    public string MarksName { get; set; }

    public Nullable<int> Applicant_Id { get; set; }

    public Nullable<decimal> TotalMarks { get; set; }

    public Nullable<decimal> ObtainedMarks { get; set; }

    public string Institute { get; set; }

    public Nullable<int> QualificationTypeId { get; set; }

    public Nullable<int> Required_Degree_Id { get; set; }

    public string OtherQualification { get; set; }

    public Nullable<System.DateTime> PassingYear { get; set; }

    public string Grade { get; set; }

    public string UploadPath { get; set; }

    public string PmdcPath { get; set; }

    public Nullable<System.DateTime> PmdcFrom { get; set; }

    public Nullable<System.DateTime> PmdcTo { get; set; }

    public Nullable<System.DateTime> DegreeFrom { get; set; }

    public Nullable<System.DateTime> DegreeTo { get; set; }

    public string DegreeName { get; set; }

    public string DegreeType { get; set; }

    public Nullable<System.DateTime> ExpFrom { get; set; }

    public Nullable<System.DateTime> ExpTo { get; set; }

    public string HealthFacility { get; set; }

    public Nullable<bool> UploadedInGrievance { get; set; }

    public Nullable<System.DateTime> GrievanceDatetime { get; set; }

    public Nullable<int> ScrutinyId { get; set; }

    public Nullable<int> Old_Id { get; set; }

    public Nullable<int> BatchApplicationId { get; set; }

    public Nullable<bool> IsVerified { get; set; }

    public Nullable<System.DateTime> VerifiedDatetime { get; set; }

    public string VerifiedBy { get; set; }

    public string VerifiedByUserId { get; set; }

}

}
