...
@Component({
  selector: 'app-dds-file',
  templateUrl: './file.component.html',
  styles: []
})
export class DDSFileComponent implements OnInit {
  @Input() public cnic: any;
  public file: any;
  ...
  constructor(private _rootService: RootService, private sanitizer: DomSanitizer, public _notificationService: NotificationService) { }

  ngOnInit() {
    this._rootService.getProfileDetail(this.cnic, 1).subscribe((data: any) => {
      this.file = data;
  ...
  public uploadFile() {
    ...
    if (this.photoFile.length > 0) {
      this._rootService.uploadFileAttachment(this.photoFile, this.dds.Id).subscribe((res: any) => {
        if (res) {
          this.photoFile = [];
  ...
  public getFileAttachments() {
    this._rootService.getFileAttachments(this.dds.Id).subscribe((res: any) => {
      if (res) {
  ...
  public removeFileAttachments(id) {
    if (confirm('Are you sure?')) {
      this._rootService.removeFileAttachments(id).subscribe((res: any) => {
        if (res) {
          this.getFileAttachments();
        }
      }, err => {
