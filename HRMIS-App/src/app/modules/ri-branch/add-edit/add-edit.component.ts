...
export class AddEditComponent implements OnInit {
  ...
  private loadDropdownValues = () => {
    ...
    if (this.currentUser.UserName == 'pshd') {
    ...
    this._riService.submitApplication(this.application).subscribe((response: any) => {
      if (response.application) {
        ...
        this._riService.createApplicationLog(applicationLog).subscribe((response2: any) => {
          if (response2.Id) {
            this.applicationDocuments.forEach((x: any) => {
              if (x.checked) {
                let applicationAttachment: any = {};
                applicationAttachment.Application_Id = this.application.Id;
                applicationAttachment.Document_Id = x.Id;
                applicationAttachment.documentName = x.Name;
              ...
              if (this.photoFile.length > 0) {
                this._riService.uploadApplicationAttachments([{ files: this.photoFile, Document_Id: 153 }], this.application.Id).subscribe((res: any) => {
                  if (res) {
                    ...
                    this.savingApplication = false;
                    /* this._riService.submitApplicationAttachments(this.applicationAttachments).subscribe((response3: any) => {
                      this._notificationService.notify('success', 'Tracking Number : ' + this.application.TrackingNumber + ' marked to ' + this.application.ForwardingOfficerName);
                      this.savingApplication = false;
                    },
                      err => { this.handleError(err); }
  ...
  private getPandSOfficers = (type: string) => {
    ...
    this._rootService.getPandSOfficers(type).subscribe((res: any) => {
      if (type == 'fts') {
        ...
        this.punjabOfficersData = res;
        this.punjabOfficers = this.punjabOfficersData;
      }
    },
      err => { this.handleError(err); }
    );
  }
  private getPunjabOfficers() {
    this._rootService.getPunjabOfficers().subscribe((data: any) => {
      this.punjabOfficersData = data;
      this.punjabOfficers = data;
    });
  }
