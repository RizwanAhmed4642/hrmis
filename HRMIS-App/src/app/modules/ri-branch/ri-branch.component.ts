...
@Component({
  selector: 'app-ri-branch',
  templateUrl: './ri-branch.component.html',
  styles: []
})
export class RiBranchComponent implements OnInit {
  ...
  constructor(private sanitizer: DomSanitizer, private _rootService: RootService, private _riService: RiBranchService, private _authenticationService: AuthenticationService, private _notificationService: NotificationService) { }
  ...
  public getApplications() {
    ...
    this._riService.getApplications(this.kGrid.skip,
      this.kGrid.pageSize,
      this.searchQuery,
      this.sourceId,
      this.office_Id,
  ...
  public removeApplication(application) {
    if (confirm('Remove Tracking # ' + application.TrackingNumber + '?')) {
      ...
      this.kGrid.loading = true;
      this._riService.removeApplication(application.Id, application.TrackingNumber).subscribe((res: any) => {
        if (res) {
