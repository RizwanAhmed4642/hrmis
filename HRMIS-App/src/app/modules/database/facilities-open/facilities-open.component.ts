...
@Component({
  selector: "app-facilities-open",
  templateUrl: "./facilities-open.component.html",
  styles: []
})
export class FacilitiesOpenComponent implements OnInit {
  ...
  public removeOpenedHF(Id: number) {
    if (confirm('Are you sure?')) {
      this._databaseService.removeOpenHF(Id).subscribe((res: any) => {
        if (res) {
  ...
  public saveOpenHF() {
    this.saving = true;
    this.hfOpened.MeritsActiveDesignationId = 68;
    this.hfOpened.Designation_Id = 1320;
    this._databaseService.saveHFOpen(this.hfOpened).subscribe((data) => {
      if (data) {
        this.getHFOpened();
        this.hfOpened = {};
        this.hfName = '';
