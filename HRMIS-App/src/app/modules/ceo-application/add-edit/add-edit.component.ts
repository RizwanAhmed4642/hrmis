...
import { ApplicationMaster, ApplicationProfileViewModel, ApplicationLog, ApplicationAttachment } from '../../application-fts/application-fts';
...
export class AddEditComponent implements OnInit {
  ...
  public types: any[] = [{ Id: 15, Name: 'Pension Case' }];
  ...
  constructor(private sanitizer: DomSanitizer, private _notificationService: NotificationService, private _rootService: RootService, private _riService: CEOApplicationService, private _authenticationService: AuthenticationService) { }
  private loadDropdownValues = () => {
    this.getDivisions('0');
    this.getDistricts('0');
    this.getTehsils('0');
    this.getPandSOfficers('all');
    this.getApplicationDocuments(15);
    this.dropDowns.selectedFiltersModel.applicationType = { Id: 15, Name: 'Pension Case' };
    this.dropDowns.selectedFiltersModel.officer = { DesignationName: 'R & I Branch (PSH Department)', Id: 144 };
    this.application.ApplicationType_Id = 15;
    this.application.ForwardingOfficer_Id = 144;
    this.application.ForwardingOfficerName = 'R & I Branch';
  ...
  private getDistricts = (code: string) => {
    ...
    this._rootService.getDistricts(code).subscribe((res: any) => {
      ...
      let currentDistrict = this.dropDowns.districts.find(x => x.Code == this.currentUser.DistrictCode);
      this.application.DispatchFrom = currentDistrict ? ("Chief Executive Officer (" + currentDistrict.Name + ")") : '';
      this.application.DispatchFrom = currentDistrict ? ("Districts (" + currentDistrict.Name + ")") : '';
