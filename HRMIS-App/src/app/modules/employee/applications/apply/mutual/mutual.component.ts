...
export class MutualComponent implements OnInit {
  ...
  public applicationSubType_Id: number = 2;
  ...
  public verifyMutualCode() {
    this.isMutualVerified = false;
    this._employeeService.verifyMutualCode(this.mutualProfile.firstEmployee.Id, this.mutualProfile.secondEmployee.Profile_Id, this.application.MutualCodeOne, 0).subscribe((res: any) => {
      if (res == true) {
            ...
            if (response2.Id) {
              this._employeeService.checkApplications(app.HealthFacility_Id, app.ToHF_Id, app.Designation_Id).subscribe((d: any) => {
                if (d && d.applicationFromCounts) {
                  ...
                  } else if (this.applicationSubType_Id == 2) {
                    this.showMessage = 'Submitted Successfully';
                    this._notificationService.notify('success', 'Submitted Successfully.');
                  }
                }
