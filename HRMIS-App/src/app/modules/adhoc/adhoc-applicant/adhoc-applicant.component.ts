...
@Component({
  selector: "app-adhoc-applicant",
  templateUrl: "./adhoc-applicant.component.html",
  styleUrls: ['./adhoc-applicant.component.scss']
})
export class AdhocApplicantComponent implements OnInit, OnDestroy {
  ...
  public experienceContinueDate: Date = new Date('11/31/2021 23:59:59')
  ...
  public ngOnInit(): void {
    ...
    this.isAdmin = this.user.UserName == 'dpd' || this.user.UserName == 'hisdu.co2' ? true : false;
    this.fetchParams();
  ...
  public getAdhocApplicationApplicant() {
    this.documents = [];
    this._adhocService.getAdhocApplicationApplicant(this.applicant.Id ? this.applicant.Id : 0, this.application.Id ? this.application.Id : 0).subscribe((res: any) => {
      if (res) {
  ...
  public getApplicationData() {
    ...
    doc = {
      Name: 'PMC', Valid: this.applicant.PMDCValidUpto, PMDCNumber: this.applicant.PMDCNumber,
      ...
      this._adhocService.getExperiences(this.applicant.Id).subscribe((x1: any) => {
        if (x1) {
          ...
          this.applicant.ApplicantExperiences.forEach(exp => {
            ...
            if (new Date(exp.ToDate) > this.experienceContinueDate) {
              exp.continuedC = true;
            }
          });
        }
        if (this.application) {
          ...
          this._adhocService.getAdhocScrutinyByApplicant(this.applicant.Id).subscribe((x2: any) => {
            if (x2) {
    ...
    this._adhocService.getAdhocApplicationMerit(this.application.Id).subscribe((x: any) => {
      if (x) {
        this.application.interviews = x.adhocInterviews;
        this.application.merits = x.adhocMerits;
        this.application.posting = x.adhocPostings;
      }
    }, err => {
      console.log(err);
    });
  ...
  public checkScrutiny() {
    ...
    console.log(this.applicant.Qualifications);
  ...
  public uploadApplicantQualification(item: any) {
    ...
    if (item.photoFile.length > 0) {
      this._adhocService.uploadApplicantQualification(item.photoFile, item.Id).subscribe((x: any) => {
        this._adhocService.getApplicantDocuments(this.applicant.Id).subscribe((x: any) => {
          if (x) {
            this.applicant.Qualifications = x;
            this.checkScrutiny();
  ...
  public verifyHifzPosition = (item: any, status: number) => {
    item.savingP = true;
    this._adhocService.verifyHifzPosition(item.Id, 3, status).subscribe((res: any) => {
      item.savingP = false;
      this._adhocService.getApplicantDocuments(this.applicant.Id).subscribe((x: any) => {
        if (x) {
          this.applicant.Qualifications = x;
  ...
  public acceptAdhocGrievanceScrutiny(ads) {
    ads.loading = true;
    ads.GrievanceAccepted = true;
    this._adhocService.acceptAdhocGrievanceScrutiny(ads).subscribe((res) => {
      if (res) {
        this._adhocService.getExperiences(this.applicant.Id).subscribe((x1: any) => {
          if (x1) {
            this.applicant.ApplicantExperiences = x1;
