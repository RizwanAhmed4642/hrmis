...
import { Empattendance } from '../profile.class';
...
export class AddEditComponent implements OnInit, OnDestroy {
  @ViewChild('photoRef', { static: true }) public photoRef: any;
  @ViewChild('cnicFrontRef', { static: true }) public cnicFrontRef: any;
  @ViewChild('cnicBackRef', { static: true }) public cnicBackRef: any;
  public loading = true;
  ...
  constructor(private router: Router, private _rootService: RootService, private _attandanceService: AttandanceService, private _profileService: ProfileService,
    private _authenticationService: AuthenticationService, public _notificationService: NotificationService,
    private route: ActivatedRoute) { }

  ngOnInit() {
    this.empattendance = new Empattendance();
  ...
  public onSubmit() {
    ...
    this._attandanceService.saveProfile(this.profile).subscribe((response: any) => {
      if (response && response.Id != null) {
        debugger;
        this.empattendance.ProfileID = response.Id;
        this.empattendance.IndRegID = this.profile.IndRegID;
        this._attandanceService.saveEmpAttendance(this.empattendance).subscribe((response1: any) => {
          if (response1) {
            this.savingProfile = false;
          }
        });
        if (this.photoFile.length > 0) {
          this._attandanceService.uploadPhoto(this.photoFile, response.Id).subscribe((res: any) => {
            if (res) {
              this.savingProfile = false;
              this._notificationService.notify('success', 'Employee Profile Saved!');
              this.router.navigate(['/attandance/' + response.CNIC]);
            }
          }, err => {
      ...
      (params: any) => {
        if (params.hasOwnProperty('cnic')) {
          ...
          if (this.cnic && this.cnic.length == 13) {
            ...
            this.router.navigate(['attandance/new']);
          }
        } else {
  ...
  public search(value: string, filter: string) {
    if (filter == 'hfs') {
      ...
      if (value.length > 2) {
        ...
        this._attandanceService.searchHealthFacilities(value).subscribe((data) => {
  ...
  private getDepartments = () => {
    ...
    this._attandanceService.getDepartments().subscribe((res: any) => {
  ...
  private getSubDepartments = () => {
    this.dropDowns.subDepartments = [];
    this.dropDowns.subDepartmentsData = [];
    this._attandanceService.getSubDepartments().subscribe((res: any) => {
      this.dropDowns.subDepartments = res;
      this.dropDowns.subDepartmentsData = this.dropDowns.subDepartments.slice();
    },
      err => { this.handleError(err); }
    );
  }
  ...
  public dropdownValueChanged = (value, filter) => {
    ...
    if (filter == 'subDepartments') {
      this.empattendance.SubDept_ID = value.SubDept_ID;
    }
    // console.log(this.empattendance.SubDept_ID);
  ...
  private bindProfileValues() {
    ...
    //this.dropDowns.selectedFiltersModel.subDepartments = this.profile.SubDept_ID != 0 ? { Name: this.profile.SubDept_Name, Id: this.profile.SubDept_ID } : this.dropDowns.selectedFiltersModel.subDepartments;

    // this.profile.SubDept_ID = this.dropDowns.selectedFiltersModel.subDepartments.IndRegID;
    ...
    this.dropDowns.selectedFiltersModel.subDepartments = this.profile.SubDept_ID != 0 ? { DName: this.profile.DName, SubDept_ID: this.profile.SubDept_ID } : this.dropDowns.selectedFiltersModel.subDepartments;
