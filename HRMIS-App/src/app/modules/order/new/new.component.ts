...
export class NewComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.fetchParams();
    this.orderRequestId = this._localService.get('ORId');
    console.log(this.orderRequestId);
  ...
  private generateOrderBarcode() {
    if (this.application.TrackingNumber) {
      ...
      this._orderService.generateOrderQr(this.application.EmployeeName + ', ' + this.application.CNIC).subscribe((res: any) => {
        if (res && res.qrCode) {
          this.application.qrCode = res.qrCode;
        }
      }, err => {
        this.handleError(err);
      });
  ...
  private loadDropdownValues = () => {
    ...
    } else if (this.user.RoleName == 'DG Health') {
      this.getPandSOfficers('dg health');
  ...
  public editOrderRequest() {
    if (this.orderRequestId > 0) {
      this._orderService.editOrderRequest(this.esr.Id, this.orderRequestId).subscribe((response) => {
