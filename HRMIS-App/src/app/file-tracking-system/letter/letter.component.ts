...
export class LetterComponent implements OnInit, OnDestroy {
    ...
    this.subscription = this.route.params.subscribe(
      (params: any) => {
        if (params.hasOwnProperty('id') && +params['id'] && params.hasOwnProperty('tracking') && +params['tracking']) {
          ...
          this.letterTypeName = this.letterType == 1 ? 'Waiting Documents' : this.letterType == 2 ? 'Application Approved' : this.letterType == 3 ? 'Application Rejected' : this.letterType == 4 ? 'Application Disposed' : '';
  ...
  private getApplication(id, tracking) {
    this._fileTrackingSystemService.getApplicationForLetter(+id, +tracking).subscribe((data: any) => {
      if (data) {
    ...
    this.getOfficersForLetter(id, tracking);
  }
  private getOfficersForLetter(id, tracking) {
    this._fileTrackingSystemService.getOfficersForLetter(+id, +tracking).subscribe((data: any) => {
      if (data) {
        let today = new Date();
        ...
        let appLogCCs = data;
        appLogCCs.forEach(element => {
          if (element.Code) {
            if (element.Code.toString().length <= 5) {
              if (element.DesignationName.startsWith('Additional')) {
                this.CC2 = element.DesignationName;
              }
              if (element.DesignationName.startsWith('Deputy')) {
                this.signedBy = element.DesignationName;
                this.CC3 = element.DesignationName;
              }
              if (element.DesignationName.startsWith('Section')) {
                this.CC4 = element.DesignationName;
              }
            }
          }
        });
