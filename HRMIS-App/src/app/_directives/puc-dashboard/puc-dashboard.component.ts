...
@Component({
  selector: 'puc-dashboard',
  templateUrl: 'puc-dashboard.component.html',
  styles: [`
  p.hoverable-table {
...
export class PUCDashboardComponent implements OnInit, OnDestroy {
  ...
  public pucReportCustom: any[] = [];
  public fileRequisitions: any[] = [];
  public pucDashboardData: any[] = [];
  ...
  public pucDashboard() {
    ...
    this.pucDashboardData = [];
    this._dashboardService.pucDashboard({
      From: this.rangeDept.start,
      To: this.rangeDept.end,
      FixedDate: this.rangeDept.end
    }).subscribe((data: any) => {
      if (data) {
        this.pucDashboardData = data;
        console.log('pucDashboardData: ', this.pucDashboardData);
        this.setOffices();
      }
    }, err => {
      console.log(err);
  ...
  public setOffices() {
    let tempOfficerName: string = '';
    this.pucDashboardData.forEach(d => {
      let name = d.Officer;
      if (name) {
        if (tempOfficerName != name) {
          d.heading = true;
          tempOfficerName = name;
        }
        else {
          d.heading = false;
        }
        name = d.Officer.toLowerCase();
  ...
  public cellCLickedFc(item, statusId) {
    ...
    let o = pso.find(x => x.DesignationName == item.Officer);
    if (o) {
      this.pandSOfficer = o;
      this.officerId = o.Id;
    }
  ...
  public openApplicationDialog(item: any, statusId: number) {
    this.selectedOfficer = item;
    this.applicatonDialogOpened = true;
    this.cellCLickedFc(this.selectedOfficer, statusId);
  }
