...
@Component({
  selector: 'app-department-fts-pendancy-table',
  templateUrl: 'department-fts-pendancy-table.component.html',
  styles: [`
  p.hoverable-table {
...
export class DepartmentFTSPendancyTableComponent implements OnInit, OnDestroy {
  ...
  public getDepartmentReport() {
    ...
    if (this.typeId != null && this.typeId > 0) {
      this._dashboardService.dashboardPendencyCountTypes(this.rangeDept.start, this.rangeDept.end, this.program, this.typeId).subscribe((data: any) => {
        if (data) {
          this.kGrid.data = [];
  ...
  public getDashboardPendency3() {
    this.showB = false;
    this._dashboardService.getDashboardPendency3(this.rangeDept.start, this.rangeDept.end, this.program, this.type).subscribe((data: any) => {
      if (data) {
        let p = data.pendancy;
        this.kGrid.data.forEach(report => {
          report.TodayUnderProcess = 0;
          report.TodayDispose = 0;
          report.UnderProcessGT7Days = 0;
          report.UnderProcessGT15Days = 0;
          report.UnderProcessGT30Days = 0;
          report.UnderProcessUntilToday = 0;
          p.forEach(pend => {
            if (report.OfficerDesignation == pend.OfficerDesignation) {
              report.Program = pend.Program;
  ...
  public setOffices() {
    this.kGrid.data.forEach(d => {
      ...
      if (name) {
        ...
        else if (d.Program == 'South') {
          if (name.includes('section') && name.includes('d&f') && d.Program == 'South') { this.secretary.dnf = d; }
          if (name.includes('section') && name.includes('purchase') && d.Program == 'South') { this.secretary.purchase = d; }
          if (name.includes('section') && name.includes('coord') && d.Program == 'South') { this.secretary.coordSouth = d; }
          if (name.includes('section') && name.includes('dental & pharmacy') && d.Program == 'South') { this.secretary.dnp = d; }
          if (name.includes('section') && name.includes('(nc)') && d.Program == 'South') { this.secretary.ncSouth = d; }
          if (name.includes('section') && name.includes('(ahp)') && d.Program == 'South') { this.secretary.ahpSouth = d; }
          if (name.includes('section') && name.includes('(b&a)') && d.Program == 'South') { this.secretary.bnaSouth = d; }
          if (name.includes('section') && name.includes('(tech-ii)') && d.Program == 'South') { this.secretary.techiiSouth = d; }
          if (name.includes('section') && name.includes('(tech-i)') && d.Program == 'South') { this.secretary.techiSouth = d; }
          if (name.includes('section') && name.includes('(gcw)') && d.Program == 'South') { this.secretary.gcw = d; }
          if (name.includes('section') && name.includes('(gcm)') && d.Program == 'South') { this.secretary.gcm = d; }
          if (name.includes('section') && name.includes('(inquiry)') && d.Program == 'South') { this.secretary.inquiry = d; }
          if (name.includes('section') && name.includes('(general)') && d.Program == 'South') { this.secretary.general = d; }
          if (name.includes('additional') && name.includes('(admn)') && d.Program == 'South') { this.secretary.admnSouth = d; }
        }
      }
    });
