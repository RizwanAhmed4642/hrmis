...
export class ProfileComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute,
    ...
    private _authenticationService: AuthenticationService, private _rootService: RootService, private _onlineApplicationService: OnlineApplicationService) { }
  ...
  public fetchMeritInfo() {
    this._onlineApplicationService.getProfileByCNIC(this.cnic).subscribe((res: any) => {
      if (res) {
        this.profile = res;
        //this.setValues();
      }
  ...
  public onSubmit() {
   ...
   /*  this._onlineApplicationService.verifyMeritProfile(this.merit).subscribe((res: any) => {
      if (res && res.Id) {
        if (this.photoFile.length > 0) {
          this._onlineApplicationService.uploadMeritProfilePhoto(this.photoFile, res.CNIC).subscribe((res: any) => {
            if (res) {
