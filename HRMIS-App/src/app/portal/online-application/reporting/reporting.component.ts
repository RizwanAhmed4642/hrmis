...
export class ReportingApplicationComponent implements OnInit {
  ...
  constructor(public _notificationService: NotificationService,
    ...
    private _onlineApplicationService: OnlineApplicationService,
    private _authenticationService: AuthenticationService) { }


  public getApplications() {
    this.kGrid.loading = true;
  /*   this._onlineApplicationService.getApplications(
      this.kGrid.skip,
      this.kGrid.pageSize,
      this.searchQuery,
      ).subscribe(
  ...
  public openInNewTab(type: string, item: any) {
    /* if (type == 'acceptance') {
      ...
      this._onlineApplicationService.getDownloadLink('offer', item.Cnic).subscribe((link) => {
        if (link) {
