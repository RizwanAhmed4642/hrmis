...
export class AwaitingPostingComponent implements OnInit {
  ...
  public designationId: number = 362;
  ...
  public getMerits() {
    ...
    this._onlinePostingService.getMeritsByCNIC({
  ...
  public getMoWmo() {
    this.designationId = this.designationId == 1320 ? 802 : 1320;
    this.getMerits();
  }
  ...
  public search(value: string, filter: string) {
    if (filter == 'hfs') {
      ...
      if (value.length > 2) {
        ...
        this.query = value;
        console.log(value);
        console.log(this.selectedMerit.Designation_Id);
        
        this._rootService.searchVacancy(value, this.selectedMerit.Designation_Id).subscribe((data) => {
          this.hfOpened = data as any[];
          this._onlinePostingService.getAwaitingPosting(this.selectedMerit.Id).subscribe((res: any) => {
            if (res) {
              this.meritPosting = res;
              if (this.meritPosting.PostingHF_Id) {
                this.hfOpened.forEach(pref => {
                  if (pref && pref.HF_Id) {
                    if (pref.HF_Id == this.meritPosting.PostingHF_Id) {
  ...
  public selectMerit(item: any) {
    ...
    this.getPreferences();
    this.search(this.selectedMerit.DomicileName ? this.selectedMerit.DomicileName : 'Bahawalpur', 'hfs');
    //this.checkPg();
  }
  public getPreferences() {
    this.selectedMeritPreferences = [];
    this._onlinePostingService.getPreferencesList(this.selectedMerit.CNIC).subscribe((response: any) => {
      if (response) {
        ...
        this._onlinePostingService.getMeritPosting(this.selectedMerit.Id).subscribe((response2: any) => {
          if (response2) {
            ...
            this.selectedMeritPreferences.forEach(pref => {
              if (pref && pref.FullName) {
                if (pref.Id == this.meritPosting.HFName) {
                  pref.mPosting = this.meritPosting;
                } else {
                  pref.mPosting = {};
                }
  ...
  public getPreferencesOpenWindow(item: any) {
    ...
    this.preferencesWindow = true;
    this.noVacancy = false;
    if (this.selectedMerit && this.selectedMerit.DesignationId == 373) {
      this.offerDate = '27';
  ...
  public postCandidate(item) {
    ...
    this._onlinePostingService.postCandidateHF(this.selectedMerit.Id, item.HF_Id, item.HFMISCode).subscribe((response: any) => {
      if (response) {
        item.noVacancy = false;
        this.meritPosting = {};
        this.loadingPreferences = false;
        this._onlinePostingService.getAwaitingPosting(this.selectedMerit.Id).subscribe((res: any) => {
          if (res) {
            ...
            if (item.Id == 0) {
              this.selectedMeritPreferences.forEach(pref => {
                if (pref && pref.FullName) {
                  if (pref.FullName == this.meritPosting.PostingHFName) {
                    pref.mPosting = this.meritPosting;
                  } else {
                    pref.mPosting = {};
                  }
  ...
  public getPostedMerits(dataItem: any) {
    ...
    this._onlinePostingService.getPostedMerits(dataItem.HFMISCode, this.selectedMerit.DesignationId, this.selectedMerit.MeritsActiveDesignationId).subscribe((response: any) => {
  ...
  lockAwaitingPostingOrder(item) {
    item.locking = true;
    this.noVacancy = false;
    this._onlinePostingService.lockAwaitingPostingOrder(this.selectedMerit.Id).subscribe((res: any) => {
      if (res == true) {
