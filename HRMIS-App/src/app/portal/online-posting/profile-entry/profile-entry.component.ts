...
export class ProfileEntryComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    //this.subscribeCNIC();
    this.getDomiciles();
    this.dropDowns.selectedFiltersModel.designation = { Name: 'Medical Officer', Id: 802 };
  }
  private fetchParams() {
  ...
  public fetchMeritInfo() {
    this._onlinePostingService.getMeritById(this.merit.Id).subscribe((res: any) => {
      if (res) {
        this.merit = res;
        this.setValues();
  ...
  public onSubmit() {
    ...
    debugger;
    if (this.merit.DOB && this.merit.DOB instanceof Date) {
      this.merit.DOB = this.merit.DOB.toDateString();
    }
    this.merit.MeritsActiveDesignationId = 61;
    this.merit.Designation_Id = 802;
    this.merit.Status = this.merit.Status ? this.merit.Status : 'New';
    /*  this.merit.Status = 1320;  */
    /*    this._onlinePostingService.verifyMeritProfile(this.merit).subscribe((res: any) => {
        if (res && res.Id) {
          if (this.photoFile.length > 0) {
            this._onlinePostingService.uploadMeritProfilePhoto(this.photoFile, res.CNIC).subscribe((res: any) => {
              if (res) {
                this.savingProfile = false;
    ...
    this._onlinePostingService.saveProfile(this.merit).subscribe((res: any) => {
      if (res && res.Id) {
