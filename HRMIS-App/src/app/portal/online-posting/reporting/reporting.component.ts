...
export class ReportingComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getMeritActiveDesignations();
  ...
  public getPreferencesOpenWindow(item: any) {
    ...
    this._onlinePostingService.generateOrderQr(this.selectedMerit.Name + ', ' + this.selectedMerit.CNIC).subscribe((res: any) => {
      if (res && res.qrCode) {
        this.esr.qrCode = res.qrCode;
    ...
    if (this.selectedMerit) {
      this.offerDate = '14';
      this.offerMonth = 'January';
    }
    if (this.selectedMerit.MeritsActiveDesignationId == 61) {
      this._onlinePostingService.isPGInit(this.selectedMerit.Id).subscribe((x: any) => {
        if (x) {
          this.selectedMerit.pgForm = x;
          if (this.selectedMerit.pgForm.isPG) {
  ...
  public dropdownValueChanged = (value, filter) => {
    ...
    if (filter == 'designation') {
      this.ActiveDesignationId = value.Id;
      this.getMerits();
    }
  }
  ...
  public qrodeSrc() {
    return this.sanitized.bypassSecurityTrustUrl(this.esr.qrCode);
  }
  ...
  genOrder() {
    ...
    this._onlinePostingService.meritOrder(this.selectedMerit.Id, html).subscribe((res: any) => {
      if (res) {
        ...
        let qr = this.esr.qrCode;
        this.esr = res.esr;
        this.esr.qrCode = qr;
        this.profile = res.hrProfile;
        this._notificationService.notify('success', 'Order Saved');
        html = document.getElementById('applicationPrintOld').innerHTML;
      /*   if (this.esr.Id) {
