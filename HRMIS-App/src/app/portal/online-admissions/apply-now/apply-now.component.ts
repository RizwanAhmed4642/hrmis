...
export class ApplyNowComponent implements OnInit, OnDestroy {
  ...
  public sendConfimationSMS(applicationId: number) {
    this._onlineAdmissionsService.sendConfimationSMS(applicationId).subscribe((res: any) => {
      if (res) {
  ...
  public lockApplication() {
    this.locking = true;
    this._onlineAdmissionsService.lockApplication(this.application.Id).subscribe((res: any) => {
      if (res) {
  ...
  public removeHF(item) {
    item.loading = true;
    this._onlineAdmissionsService.removeApplicantPreference(item.prefId).subscribe((res: any) => {
      if (res) {
        item.applied = false;
      }
      item.loading = false;
  ...
  public getApplication() {
    this.loading = true;
    this._onlineAdmissionsService.getApplicationSingle(this.applicant.Id, this.desigId).subscribe((res: any) => {
      if (res) {
  ...
  public getapplicantDocuments() {
    this.loading = true;
    this._onlineAdmissionsService.getApplicantQualification(this.applicant.Id).subscribe((res: any) => {
      if (res) {
        this.documents = res;
        this.loading = false;
      }
