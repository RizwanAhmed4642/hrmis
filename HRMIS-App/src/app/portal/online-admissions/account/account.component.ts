...
export class AccountComponent implements OnInit, OnDestroy {
  ...
  public urdu: any = {
    ...
    fourth: `
کسی بھی قسم کی دشواری کی صورت میں ہیلپ لائن نمبر 92 42 9200965 سے رہنمائی حاصل کی جاسکتی ہے۔
`,
    fifth: `
موبائل نمبر یا ای میل ایڈریس تبدیل کروانے کیلئے ہیلپ لائن نمبر 1033 سے رجوع کریں۔
`,
    fiftheng: 'To change your mobile number or email address, please contact helpline 1033',
  };
  public profile: any;
  ...
  public checkUSer() {
    ...
    if (this.user.Cnic && this.user.Cnic[12] != ' ') {
      ...
      this._onlineAdmissionsService.getUser(this.user.Cnic).subscribe((res: any) => {
        if (res == false) {
          this._onlineAdmissionsService.getUserFull(this.user.Cnic).subscribe((res: any) => {
            if (res) {
              this.searchedUser = res;
              this.user.PhoneNumber = this.searchedUser.PhoneNumber;
              this.user.Email = this.searchedUser.Email;
              if (this.searchedUser.UserDetail == 'Job Applicant Adhoc Portal User.') {
  ...
  private getJobs = () => {
    this.jobs = [];
    this._onlineAdmissionsService.getJobs().subscribe((res: any) => {
      if (res) {
        this.jobs = res;
      }
    },
  ...
  public checkPortalActive(id) {
    this.checking = true;
    this._onlineAdmissionsService.getMeritActiveDesignation(id).subscribe((res: any) => {
  ...
  public getPassword() {
    ...
    this.errors = [];
    this.user.roles = [];
    this.user.roles.push('JobApplicantAdhoc');
    this.user.isUpdated = true;
    this.user.UserName = this.user.Cnic;
    this.user.PhoneNumber = this.user.PhoneNumber;
    this.user.UserDetail = 'Job Applicant Adhoc Portal User.';
    this.user.ConfirmPassword = this.user.Password;
    this._onlineAdmissionsService.getPassword(this.user).subscribe((response: any) => {
      this.generatingPassword = false;
  ...
  public fetchMeritInfo() {
    this._onlineAdmissionsService.getMeritProfile(this.user.Cnic).subscribe((res: any) => {
      if (res && res.Id) {
  ...
  public login() {
    this.sigingin = true;
    this.loginError = '';
    this._onlineAdmissionsService.login({ username: this.user.Cnic, password: this.user.password }).subscribe((res) => {
      if (res) {
        let setuser = this.setUserPHFMC(res);
        console.log(setuser);

        this._onlineAdmissionsService.getApplicant(this.user.Cnic).subscribe((res2: any) => {
          if (res2) {
            console.log(res2);

            /*  if (res2.Status_Id == 2) {
              this._onlineAdmissionsService.getApplicantDocuments(res2.Id).subscribe((res3: any) => {
                if (res3) {
