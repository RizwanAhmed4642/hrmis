...
export class OfferLetterComponent implements OnInit, OnDestroy {
  ...
  public uploadJobApplicantAttachments() {
    this.router.navigate(['/adhoc/experience']);
/*     if (this.applicationAttachments.length > 0) {
      ...
      this._onlineAdmissionsService.uploadJobApplicantAttachments(this.applicationAttachments, this.cnic).subscribe((res) => {
        if (res) {
          this.router.navigate(['/adhoc/experience']);
        }
      }, err => {
  ...
  public readUrl(event: any, filter: string) {
    if (event.target.files && event.target.files[0]) {
      if (filter == 'pic') {
        ...
        this._onlineAdmissionsService.uploadSignedAcceptance(this.photoFile, this.applicant.Id).subscribe((x: any) => {
          if (x.result) {
