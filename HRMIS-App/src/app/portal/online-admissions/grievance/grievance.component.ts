...
export class GrievanceComponent implements OnInit, OnDestroy {
  ...
  public getApplicationGrievances() {
    this.loading = true;
    this._onlineAdmissionsService.getApplicationGrievances(this.applicant.Id).subscribe((res: any) => {
      if (res) {
        this.grievances = res;
        console.log('grv: ', res);
        this.loading = false;
  ...
  public removeGrievance(Id) {
    this._onlineAdmissionsService.removeGrievance(Id).subscribe((res: any) => {
      if (res) {
        this.getApplicationGrievances();
      }
    }, err => {
