...
export class ProfileComponent implements OnInit {
  ...
  private getApplicant() {
    this._onlineAdmissionsService.getApplicant(this.cnic).subscribe((res: any) => {
      if (res) {
  ...
  private getAdhocApplicationByDesig() {
    this._onlineAdmissionsService.getAdhocApplicationByDesig(this.applicant.Id, this.designationId).subscribe((res: any) => {
      if (res) {
  ...
  private getJob() {
    ...
    this._onlineAdmissionsService.getJobByDesignation(designationId).subscribe((res: any) => {
      if (res) {
        this.job = res;
      }
    }, err2 => {
  ...
  public uploadFile(filter: string) {
    if (filter == 'photo' && this.photoFile.length > 0) {
      ...
      this._onlineAdmissionsService.uploadApplicantPhoto(this.photoFile, this.applicant.CNIC).subscribe((res: any) => {
        if (res) {
          this.getApplicant();
        }
      }, err => {
    ...
    if (filter == 'pmdc' && this.pmdcFile.length > 0) {
      ...
      this._onlineAdmissionsService.uploadApplicantPMDC(this.pmdcFile, this.applicant.CNIC).subscribe((res: any) => {
        if (res) {
          this.getApplicant();
        }
      }, err => {
    ...
    if (filter == 'cnic' && this.cnicFile.length > 0) {
      ...
      this._onlineAdmissionsService.uploadApplicantCNIC(this.cnicFile, this.applicant.CNIC).subscribe((res: any) => {
        if (res) {
          this.getApplicant();
        }
      }, err => {
    ...
    if (filter == 'domicile' && this.domicileFile.length > 0) {
      ...
      this._onlineAdmissionsService.uploadApplicantDomicile(this.domicileFile, this.applicant.CNIC).subscribe((res: any) => {
        if (res) {
          this.getApplicant();
        }
      }, err => {
    ...
    if (filter == 'hifz' && this.hifzFile.length > 0) {
      ...
      this._onlineAdmissionsService.uploadApplicantHifz(this.hifzFile, this.applicant.CNIC).subscribe((res4: any) => {
        this.getApplicant();
      }, err => {
        this.handleError(err);
      });
  ...
  public onSubmit() {
    ...
    this._onlineAdmissionsService.saveApplicant(this.applicant).subscribe((res: any) => {
      if (res && res.Id) {
        this.basicInfoSaved = true;
        let fileCheck: number = 0;
        /*  if (this.photoFile.length > 0) {
           this._onlineAdmissionsService.uploadApplicantPhoto(this.photoFile, res.CNIC).subscribe((res2: any) => {
             fileCheck += 1;
           }, err => {
             this.handleError(err);
           });
         ...
         if (this.hifzFile.length > 0) {
           this._onlineAdmissionsService.uploadApplicantHifz(this.hifzFile, res.Id).subscribe((res4: any) => {
             fileCheck += 1;
           }, err => {
             this.handleError(err);
           });
         ...
         if (this.pmdcFile.length > 0) {
           this._onlineAdmissionsService.uploadApplicantPMDC(this.pmdcFile, res.Id).subscribe((res3: any) => {
             fileCheck += 1;
           }, err => {
             this.handleError(err);
           });
         ...
         if (this.domicileFile.length > 0) {
           this._onlineAdmissionsService.uploadApplicantDomicile(this.domicileFile, res.Id).subscribe((res3: any) => {
             fileCheck += 1;
           }, err => {
             this.handleError(err);
           });
         ...
         if (this.cnicFile.length > 0) {
           this._onlineAdmissionsService.uploadApplicantCNIC(this.cnicFile, res.Id).subscribe((res3: any) => {
             fileCheck += 1;
           }, err => {
             this.handleError(err);
           });
        ...
        this._onlineAdmissionsService.saveAdhocApplication(this.application).subscribe((res3: any) => {
          if (res3 && res3.Id) {
