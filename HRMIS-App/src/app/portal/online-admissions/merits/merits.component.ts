...
@Component({
  selector: 'merits-adhoc',
  templateUrl: './merits.component.html',
  styleUrls: ['./merits.component.scss']
})
export class MeritsComponent implements OnInit, OnDestroy {
  ...
  public meritListSelected: boolean = false;
  ...
  public getAdhocMLVL(dc: string, dsg: number) {
    this.loading = true;
    this.meritListSelected = true;
    this._onlineAdmissionsService.getAdhocMLVL(dc, dsg).subscribe((res: any) => {
      if (res) {
        this.merits = res;
        this.loading = false;
      }
  ...
  public getAdhocInterviewApplications() {
    this.loading = true;
    this._onlineAdmissionsService.getAdhocPresentApplications(this.applicant.Id).subscribe((res: any) => {
      if (res) {
