...
export class QualificationComponent implements OnInit, OnDestroy {
  ...
  public getApplicantQualification() {
    this._onlineAdmissionsService.getApplicantQualification(this.applicant.Id).subscribe((data: any) => {
      this.getDegrees();
      if (data) {
  ...
  public submitHouseJob() {
    ...
    this._onlineAdmissionsService.saveApplicantQualification(this.serviceHistory).subscribe((res: any) => {
      if (res.Id) {
        if (this.photoFile.length > 0) {
          this._onlineAdmissionsService.uploadApplicantQualification(this.photoFile, res.Id).subscribe((x: any) => {
            if (!x) {
  ...
  private getDegrees = () => {
    ...
    this._onlineAdmissionsService.getDegrees(this.designationId).subscribe((res: any) => {
      this.setDegrees(res);
  ...
  public readUrlAndUpload(event: any, dataItem: any) {
    if (event.target.files && event.target.files[0]) {
      ...
      this._onlineAdmissionsService.uploadServiceAttachement(dataItem.photoFile, dataItem.Id).subscribe((x: any) => {
        if (!x) {
  ...
  public removeApplicantQualification(item: any) {
    if (confirm('Confirm Remove Qualification?')) {
      item.removing = true;
      this._onlineAdmissionsService.removeApplicantQualification(item.Id).subscribe((data: any) => {
        this.getApplicantQualification();
        item.removing = false;
      },
        err => {
