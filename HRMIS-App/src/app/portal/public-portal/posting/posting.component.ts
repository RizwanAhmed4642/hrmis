...
export class JobPostingComponent implements OnInit {
  ...
  public dropdownValueChanged(value: any, filter: any) {
    if (filter == 'designation') {
      ...
      console.log(value);
      this.selectedJobId = value.Id;
      this.filters.JobId = this.selectedJobId;
      this.getHrApplications();
    }
    if (filter == 'batch') {
  ...
  public getHrApplications() {
    this.kGrid.loading = true;
    this._publicPortalService.getHrApplications(this.filters).subscribe((res: any) => {
      if (res) {
        console.log(res);
  ...
  public selectApplicant(id) {
    this._publicPortalService.getHrApplication(id).subscribe((res: any) => {
      if (res) {
        this.selectedApplicant = res;
        this._rootService.getProfileDetail(this.selectedApplicant.CNIC, 2).subscribe((data: any) => {
        ...
        this._rootService.getProfileDetail(this.selectedApplicant.CNIC, 0).subscribe((data: any) => {
          this.profile = data;
        },
          err => {
            console.log(err);
          });
        this._rootService.getProfileDetail(this.selectedApplicant.CNIC, 6).subscribe((data: any) => {
          this.leaveRecord = data;
        },
          err => {
            console.log(err);
          });
        this._rootService.getProfileDetail(this.selectedApplicant.CNIC, 7).subscribe((data: any) => {
          this.serviceHistory = data;
        },
          err => {
            console.log(err);
          });
        this._rootService.getProfileDetail(this.selectedApplicant.CNIC, 11).subscribe((data: any) => {
