...
export class ProfileEntryComponent implements OnInit {
  ...
  public fetchMeritInfo() {
    this._onlineAdhocService.getMeritById(this.merit.Id).subscribe((res: any) => {
      if (res) {
        this.merit = res;
        this.setValues();
  ...
  public onSubmit() {
    ...
    /*    this._onlineAdhocService.verifyMeritProfile(this.merit).subscribe((res: any) => {
        if (res && res.Id) {
          if (this.photoFile.length > 0) {
            this._onlineAdhocService.uploadMeritProfilePhoto(this.photoFile, res.CNIC).subscribe((res: any) => {
              if (res) {
                this.savingProfile = false;
    ...
    this._onlineAdhocService.saveProfile(this.merit).subscribe((res: any) => {
      if (res && res.Id) {
