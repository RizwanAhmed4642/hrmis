...
export class ExperienceComponent implements OnInit, OnDestroy {
  ...
  public removeExperience(item: any) {
    if (confirm('Are you sure?')) {
      item.removing = true;
      this._onlineAdhocService.removeExperience(item.Id).subscribe((res3: any) => {
        item.removing = false;
        if (res3) {
          this.getExperiences();
        }
  ...
  public uploadFile(experienceId: number) {
    ...
    this._onlineAdhocService.uploadExperienceCertificate(this.photoFile, experienceId).subscribe((x: any) => {
      if (!x) {
  ...
  public addExperience(obj) {
    ...
    this._onlineAdhocService.saveExperience(obj).subscribe((res: any) => {
      if (res && res.Id) {
        if (this.photoFile.length > 0) {
          this.uploadingFile = true;
          this.addingExperience = true;
          this._onlineAdhocService.uploadExperienceCertificate(this.photoFile, res.Id).subscribe((x: any) => {
            if (!x) {
              this.uploadingFileError = true;
            }
            this.uploadingFile = false;
