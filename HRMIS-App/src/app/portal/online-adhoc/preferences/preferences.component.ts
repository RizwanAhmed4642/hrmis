...
export class PreferencesComponent implements OnInit, OnDestroy {
  ...
  public changePreferenceOrder(item) {
    item.loading = true;
    this._onlineAdhocService.changePreferenceOrder(item.Id, item.PreferenceOrder).subscribe((x: any) => {
      if (x) {
        item.loading = false;
        this.getAdhocApplicationPref();
      }
  ...
  public removeHF(item) {
    item.loading = true;
    this._onlineAdhocService.removeApplicantPreference(item.Id).subscribe((res: any) => {
      if (res) {
        this.getAdhocApplicationPref();
      }
      item.loading = false;
