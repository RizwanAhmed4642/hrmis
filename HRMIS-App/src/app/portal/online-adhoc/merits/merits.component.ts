...
export class MeritsComponent implements OnInit, OnDestroy {
  ...
  public getAdhocMLVL(dc: string, dsg: number) {
    ...
    this._onlineAdhocService.getAdhocMLVL(dc, dsg).subscribe((res: any) => {
      if (res) {
        this.merits = res;
        this.loading = false;
      }
  ...
  public getAdhocInterviewApplications() {
    this.loading = true;
    this._onlineAdhocService.getAdhocPresentApplications(this.applicant.Id).subscribe((res: any) => {
      if (res) {
