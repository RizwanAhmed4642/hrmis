...
/*     if (this.applicationAttachments.length > 0) {
      ...
      this._onlineAdhocService.uploadJobApplicantAttachments(this.applicationAttachments, this.cnic).subscribe((res) => {
        if (res) {
          this.router.navigate(['/adhoc/experience']);
        }
      }, err => {
  ...
  public readUrl(event: any, filter: string) {
    if (event.target.files && event.target.files[0]) {
      if (filter == 'pic') {
        ...
        this._onlineAdhocService.uploadSignedAcceptance(this.photoFile, this.applicant.Id).subscribe((x: any) => {
          if (x.result) {
